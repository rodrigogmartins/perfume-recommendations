def calculate_affinity_score(perfume, user_preferences):
    ...
    score += len(liked_notes & perfume_notes) * 3
    score += len(liked_accords & perfume_accords) * 2
    ...
    score -= len(not_liked_notes & perfume_notes) * 5
    score -= len(not_liked_accords & perfume_accords) * 4
